sap.ui.define([
	...
	//FIN
	return Controller.extend("AR_DP_ADMIN_CREACIONRECLAMO_RASA.AR_DP_ADMIN_CREACIONRECLAMO_RASA.controller.CreacionReclamo", {
			...
			$.ajax({
				...
				success: function (dataG, textStatus, jqXHR) {
					...
					if (flagc === true && flagi === false) {
						...
						var obj = {
							codigo: "400",
							descripcion: incorrecto.length + " MATERIALES YA FUERON RECLAMADOS  "
						};
						arrmensaje.push(obj);
						...
						var obj = {
							...
							descripcion: " SE HAN RECLAMADO  " + correcto.length + " MATERIALES CORRECTAMENTE Y  " + incorrecto.length +
								" MATERIALES YA FUERON RECLAMADOS "
						};
						arrmensaje.push(obj);
		...
		contadorcaracteres: function () {
			console.log(oView.byId("entrega").getValue().length);
			if (oView.byId("entrega").getValue().length === 100) {
